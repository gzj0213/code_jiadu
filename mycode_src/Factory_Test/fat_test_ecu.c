...
typedef struct __LANINIT
{
    char eth0_ip[20];
    char eth0_mask[10];
    char eth0_gateway[20];
    char eth1_ip[20];
    char eth1_mask[10];
    char eth1_gateway[20];
} LANINIT;
LANINIT lan_conf;
...
int set_serial_protocol(char type[])
{
    static int i, ret = 1;
    static int fd;
    ...
    if (fd < 0)
    {
        printf("open /dev/rockchip-gpio failed\n");
        return -1;
    }
    else
    {
        ...
        {
            write_gpio(uart7.gpioa, 0, fd);
            write_gpio(uart7.gpiob, 0, fd);
            write_gpio(uart8.gpioa, 0, fd);
            write_gpio(uart8.gpiob, 0, fd);
        }

        else if (strcmp(type, "485") == 0 || strcmp(type, "2") == 0)
        {
            write_gpio(uart7.gpioa, 1, fd);
            write_gpio(uart7.gpiob, 0, fd);
            write_gpio(uart8.gpioa, 1, fd);
            write_gpio(uart8.gpiob, 0, fd);
        }
        ...
        {
            write_gpio(uart7.gpioa, 1, fd);
            write_gpio(uart7.gpiob, 1, fd);
            write_gpio(uart8.gpioa, 1, fd);
            write_gpio(uart8.gpiob, 1, fd);
        }
...
int network_test(void)
{
    ...
    // static int *pret;
    static char eth0_conf[200];
    static char eth1_conf[200];
    static int *pret;

    windows_info("--------------网络测试开始--------------\n");

    GetIniKeyString("lan_test", "eth0_ip", lan_conf.eth0_ip);
    GetIniKeyString("lan_test", "eth0_mask", lan_conf.eth0_mask);
    GetIniKeyString("lan_test", "eth0_gateway", lan_conf.eth0_gateway);
    GetIniKeyString("lan_test", "eth1_ip", lan_conf.eth1_ip);
    GetIniKeyString("lan_test", "eth1_mask", lan_conf.eth1_mask);
    GetIniKeyString("lan_test", "eth1_gateway", lan_conf.eth1_gateway);

    system("sudo rm /etc/NetworkManager/system-connections/*");
    system("sudo systemctl restart NetworkManager");

    sprintf(eth0_conf, "sudo nmcli con add type ethernet con-name \"Wired Connection 1\" ifname eth0 ipv4.method manual ipv4.addresses %s/%s gw4 %s", lan_conf.eth0_ip, lan_conf.eth0_mask, lan_conf.eth0_gateway);
    sprintf(eth1_conf, "sudo nmcli con add type ethernet con-name \"Wired Connection 2\" ifname eth1 ipv4.method manual ipv4.addresses %s/%s gw4 %s", lan_conf.eth1_ip, lan_conf.eth1_mask, lan_conf.eth1_gateway);
    ...
    system(eth0_conf);
    // system(eth1_conf);
    system("sudo nmcli c reload");
    system("sudo nmcli con up \"Wired Connection 1\"");
    system("sudo systemctl restart NetworkManager");
    system(eth1_conf);
    system("sudo nmcli c reload");
    system("sudo nmcli con up \"Wired Connection 2\"");
    system("sudo systemctl restart NetworkManager");
...
int uart_test(void)
{
    ...
    printf("请选择测试的串口类型:(1:主板多协议兼容串口,2:扩展232协议串口,3:全部测试):");
    scanf("%d", &uart_sel);
    if (uart_sel == 1)
    {
        printf("请输入串口模式:");
        scanf("%s", uart_mode);
        ret = set_serial_protocol(uart_mode);
        if (ret == 0)
        {
            return 0;
        }
...
int ssd_test(void)
{
    ...
    if (access("fio", F_OK) == -1)
    {
        ret = fail_info();
        return ret;
    }
    system("sudo chmod 777 fio");
...
int sata_test(void)
{
    static FILE *_satafp = NULL;
    static char sata_buf[1024];
    static int ret;

    windows_info("--------------SATA硬盘测试开始--------------\n");

    if ((_satafp = popen("df -h|grep /dev/sda1", "r")) != NULL)
    {
        while (fgets(sata_buf, 1024, _satafp) != NULL)
        {
            debug_info(sata_buf);
        }
        pclose(_satafp);
    ...
    if (strlen(sata_buf) == 0)
    {
        debug_info("(FAILED)SATA硬盘挂载失败!\n");
        ret = fail_info();
        return ret;
    }
    else
    {
        debug_info("(SUCCESS)SATA硬盘挂载成功!\n");
    }
    ...
    if ((_satafp = popen("sudo ./fio -filename=/dev/sda1 -ioengine=psync -direct=1 -thread=6 -rw=randwrite \
            -iodepth=64 -bs=4k -size=200G -numjobs=30 -runtime=60 -allow_mounted_write=1 -name=fio_test -group_reporting",
                         "r")) != NULL)
    {
        while (fgets(sata_buf, 1024, _satafp) != NULL)
    ...
    if ((_satafp = popen("sudo ./fio -filename=/dev/sda1 -ioengine=psync -direct=1 -thread=6 -rw=randread \
             -iodepth=64 -bs=4k -size=200G -numjobs=30 -runtime=60 -allow_mounted_write=1 -name=fio_test -group_reporting",
                         "r")) != NULL)
    {
        while (fgets(sata_buf, 1024, _satafp) != NULL)
    ...
    if ((_satafp = popen("echo y|sudo mkfs.ext4 /dev/sda1", "r")) != NULL)
    {
        while (fgets(sata_buf, 1024, _satafp) != NULL)
        {
            debug_info(sata_buf);
    ...
    windows_info("(SUCCESS)SATA硬盘测试成功\n");
    return 0;
}
...
int usb3_test(void)
{
    static char usbfile1[50], usbfile2[50];
    static int ret;

    windows_info("--------------USB3.0功能测试开始--------------\n");

    /*ret = system("sudo mount /dev/sdb1 /mnt/udisk/");
    if (ret != 0)
    {
        printf("请检查usb设备\n");
        ret = fail_info();
    ...
    ret = system("sudo mount /dev/sdc1 /mnt/usb_storage");
    if (ret != 0)
    {
        printf("请检查usb设备\n");
        ret = fail_info();
    ...
    ret = GetIniKeyString("setting", "usbfile1", usbfile1);
    if (ret == 0)
    {
        printf("读取的USB文件为:%s\n", usbfile1);
    }
    ...
    ret = GetIniKeyString("setting", "usbfile2", usbfile2);
    if (ret == 0)
    {
        printf("读取的USB文件为:%s\n", usbfile2);
    }
    ...
    if ((access(usbfile2, F_OK) != -1) && (access(usbfile1, F_OK) != -1))
    {
        printf("文件%s和%s存在\n", usbfile1, usbfile2);
        windows_info("(SUCCESS)USB3.0功能测试成功!\n");
        system("sudo umount /dev/sdb1");
        system("sudo umount /dev/sdc1");
        return 0;
    }
    else
    {
        if (access(usbfile1, F_OK) == -1)
        {
            printf("文件%s不存在\n", usbfile1);
        }
        else if (access(usbfile1, F_OK) == -1)
        {
            printf("文件%s不存在\n", usbfile2);
        }
        else if ((access(usbfile1, F_OK) == -1) && (access(usbfile1, F_OK) == -1))
        {
            printf("文件%s和%s不存在\n", usbfile1, usbfile2);
        }
        windows_info("(FAILED)USB3.0功能测试失败!\n");
        ret = fail_info();
        return ret;
    }
}
...
int usb2_test(void)
{
    ...
    windows_info("--------------USB2.0功能测试开始--------------\n");
    printf("请对主板上的usb2.0接口进行拔插测试!\n");

    printf("测试过程中USB2.0接口是否均正常(Y or N):");
    scanf("%s", &sel);
    if (sel == 'Y' || sel == 'y')
    {
        windows_info("(SUCCESS)USB2.0测试成功!\n");
        return 0;
    }
    else
    {
        windows_info("(FAILED)USB3.0测试失败!\n");
        ret = fail_info();
        return ret;
    }
}
...
int audio_test(void)
{
    ...
    windows_info("--------------音频功能测试开始--------------\n");

    printf("是否开启录音(Y or N):");
    scanf("%s", &sel);
    if (sel == 'Y' || sel == 'y')
    {
        // system("pactl set-source-port alsa_input.platform-es8388-sound.HiFi__hw_rockchipes8388__source \"[In] Mic\"");
        system("arecord -d 10 -c 2 -r 44100 -f cd mic.wav &");
        // usleep(100000);
        // sleep(1);
        // system("pactl set-source-port alsa_input.platform-es8388-sound.HiFi__hw_rockchipes8388__source \"[In] Headset\"");
    }
    sleep(10);

    printf("是否播放录音音频(Y or N):");
    scanf("%s", &sel);
    if (sel == 'Y' || sel == 'y')
    {
        system("aplay -D hw:2,0 mic.wav");
    }

    while (1)
    {
        printf("请检查音频是否播放正常(Y or N):");
        scanf("%s", &sel);
        if (sel == 'Y' || sel == 'y')
        {
            windows_info("(SUCCESS)音频功能测试成功!\n");
            return 0;
        }
        else if (sel == 'N' || sel == 'n')
        {
            printf("是否重听录音音频(Y or N):");
            scanf("%s", &sel);
            if (sel == 'Y' || sel == 'y')
            {
                system("aplay -D hw:2,0 mic.wav");
...
int minipcie_test(void)
{
    static FILE *_minipciefp = NULL;
    static char minipcie_buf[1024];
    int ret;

    windows_info("--------------minipcie功能测试开始--------------\n");

    timeout_flag == 0;

    system("sudo tar zxvf /home/linaro/ec20.tar.gz -C /home/linaro");
    signal(SIGALRM, timeout_handler);
    alarm(10);

    if ((_minipciefp = popen("sudo /home/linaro/ec20/ec20.sh", "r")) != NULL)
    {
        while (fgets(minipcie_buf, 1024, _minipciefp) != NULL)
        {
            if (timeout_flag == 1)
            {
                pclose(_minipciefp);
                _minipciefp = NULL;
                windows_info("(FAILED)minipcie测试失败!\n");
                system("sudo rm -r /home/linaro/ec20");
                ret = fail_info();
                return ret;
            }

            if (strcmp(minipcie_buf, "ok\n") == 0)
            {
                alarm(0);
                debug_info(minipcie_buf);
                windows_info("(SUCCESS)minipcie测试成功!\n");
                system("sudo rm -r /home/linaro/ec20");
                pclose(_minipciefp);
                _minipciefp = NULL;
                return 0;
