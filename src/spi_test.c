...
static uint8_t default_test[2] = {0xFF, 0x00};
...
static void transfer_test(int fd, uint8_t *tx, uint8_t const *rx, size_t len)
{
	int ret;
	int out_fd;
	struct spi_ioc_transfer tr = {
...
int main(int argc, char *argv[])
{
	...
	else if (transfer_size)
	{
		...
		{
			transfer_test(fd, default_test, default_rx, sizeof(default_test));
			/*if ((default_rx[0] == 0x040 && default_rx[1] == 0x18) == 1)
			{
				printf("i=%d\n", i);
			}
			else
